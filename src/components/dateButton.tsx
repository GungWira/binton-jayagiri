...
export default function DateButton({ data, dataUser }: any) {
  ...
  return (
    <>
      <div
        ref={boxDateRef}
        className="overflow-hidden h-10 box-date border border-[#0EAF7D] rounded-md mt-2 px-4"
        style={{
          filter: "drop-shadow(0px 4px 12px rgba(25, 168, 123, 0.05)",
          transition: "ease-in-out 1s",
        }}
      >
        <button
          onClick={openDateList}
          className="relative flex flex-col w-full justify-between items-center py-3"
        >
        ...
        <div className="flex flex-col w-full justify-start items-start p-2 pl-6 gap-3 bottom-1/2 border-t">
          {dateList.map((date: any, index: number) => (
            <button
              ...
              key={index}
              className="font-ligth text-xs text-[#434343]"
            >
              {date}
            </button>
          ))}
