    ...
    <>
      <div className="flex flex-col w-full">
        ...
        <p className="font-ligth text-xs text-[#434343]">
          Kamu punya orderan aktif!
        </p>
        {data.map((item: any, index: number) => {
          return (
            <div
              key={index}
              className="flex flex-row justify-between items-center border p-4 rounded mt-2"
            >
              <div className="flex flex-col justify-center gap-1">
                <p className="font-bold text-sm text-[#434343]">
                  Lapangan {item.court}
                </p>
                <p className="font-light text-xs text-[#434343]">
                  {item.start} - {item.end}
                </p>
              </div>
              <p className="font-base text-xs text-[#434343]">
                {dateList[index]}
              </p>
            </div>
          );
