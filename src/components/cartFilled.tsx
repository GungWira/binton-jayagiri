...
function formatDate(date : any) {
  ...
  return `${data[2]} ${months[data[1] === 10 ? 10 : parseInt(data[1].slice(0)) - 1]} ${data[0]}`;
}

export default function CartPageFilled({data} : any){

  const paymentHandler =  async(order_id : string, gross_amount : number) =>{
    const req = await fetch(`${process.env.NEXT_PUBLIC_REQ_URL}/pending-payment`, {
      method : "POST",
      headers : {
    ...
    <>
      <section className="">
        <div className="flex flex-col w-full">
          ...
          <p className="font-ligth text-xs text-[#434343]">Wah ada beberapa item di keranjang belanja!</p>
        </div>
            ...
            return(
              <div className="flex flex-col w-full border p-3" key={key}>
                <div className="flex flex-row w-full justify-between items-center mb-2">
                  <p className="font-bold text-base text-[#434343]">{item.id.slice(0,14)}</p>
                  <div className="p-1" style={{backgroundColor : back}}>
                    <p className="font-bold text-md" style={{color : font}}>{item.orderStatus}</p>
                  </div>
                </div>
                <div className="flex flex-row gap-2 justify-start items-center mb-2">
                  ...
                  <p className="font-bold text-base text-[#434343]">IDR {item.books.length * 20}.000</p>
                </div>
                <p className="text-base text-[#434343] mb-2">{formatDate(item.createdAt)}</p>
                {item.books.map((book : any, index : number) =>{
                  return(
                    <div className="flex flex-col w-full gap-2 pl-4" key={index}>
                      <div className="flex flex-row justify-between items-center">
                        <p className="">Lap {book.court}</p>
                        <p className="">({book.start} - {book.end})</p>
                ...
                {item.orderStatus === "pending" ? 
                  <button className="w-full p-3 bg-[#0EAF7D] mt-3 text-[#ffffff]" onClick={() => {paymentHandler(item.id, item.books.length)}}>Bayar Sekarang</button>
                :
                  <></>
                }
              </div>
