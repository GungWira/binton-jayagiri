  ...
  const orderHandler = (book: Order) => {
    ...
    if (isOrdered) {
      setOrders((prevOrders) =>
        prevOrders.filter((order: Order) => order.id !== book.id)
      );
    } else {
  ...
  return (
    <>
      <div className="mt-3 flex flex-row w-full gap-2 pb-20">
        {courts.map((court: any, index: number) => {
          ...
          return (
            ...
            >
              <p className="font-bold text-sm text-[#434343] mb-2">
                Lap {court.name}
              </p>
              {playTimes.map((playTime: any, lock: number) => {
                ...
                const isOrder = orders.some(
                  (order) => order.id === playCourt.id
                );
                ...
                if (playTime.status === 0) {
                  return (
                    <button
                      key={lock}
                      className="bg-[#16AA7C] mb-1 py-2 border rounded-full border-[#16AA7C] w-full flex justify-center items-center"
                    >
                  ...
                  return (
                    <button
                      key={lock}
                      className="mb-1 py-2 border rounded-full w-full flex justify-center items-center"
                      style={{ borderColor: border }}
                      onClick={() => {
                        orderHandler(playCourt);
                      }}
                    >
      ...
      <button
        ...
        ref={checkoutBtnRef}
        className="checkoutBtn flex flex-row justify-between w-full bg-[#0EAF7D] py-2 px-6 rounded-full items-center justify-self-end self-end fixed bottom-10"
        style={{ width: "calc(100% - 3rem)", transition: ".8s ease-in-out" }}
      >
      ...
      <section
        ref={sectionRef}
        className="sectionOrder p-6 bg-[#fefefe] fixed w-full left-0 bottom-0 py-8 pt-4 flex flex-col gap-2 justify-center items-center"
        style={{
      ...
      >
        <button
          onClick={handleOpenOrder}
          className="bg-[#434343] w-6 h-1 rounded-lg"
        ></button>
        <p className="text-[#434343] font-bold text-md pb-4 w-full">
          Detail Pemesanan
        </p>
        ...
        <div className="flex flex-row justify-between items-center w-full">
          ...
          <p className="text-[#434343] font-bold text-xs">
            {orders.length} Jam
          </p>
        </div>
        ...
        <div className="flex flex-col w-full pl-3 gap-1">
          {orders.map((order, index) => (
            <div
              ...
              key={index}
            >
              <p className="text-[#434343] font-ligth text-xs">
                Lapangan {order.name}
              </p>
              <p className="text-[#434343] font-ligth text-xs">
        ...
        >
          <div className="flex flex-col justify-center items-start">
            ...
            <p className="font-ligth text-xs text-[#ffffff]">
              Lanjutkan pemesanan sebesar IDR {orders.length * 20}.000
            </p>
          </div>
          <Image src={Kock} alt="Kock" />
        </button>
